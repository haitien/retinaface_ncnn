#ifndef RETINA_FACE_H_
#define RETINA_FACE_H_
...
const char *model_path = "/home/haitien/Music/my_retinaface_ncnn/model/mnet.25-opt.bin";
const char *param_path = "/home/haitien/Music/my_retinaface_ncnn/model/mnet.25-opt.param";
...
static inline void scale_face_box(FaceObject &obj,
                                  float h_scale,
                                  float w_scale)
{
    obj.rect.width *= w_scale;
    obj.rect.height *= h_scale;
}
static void scale_face_boxes(std::vector<FaceObject> &objs,
                             float h_scale,
                             float w_scale)
{
    for (size_t i = 0; i < objs.size(); ++i)
    {
        scale_face_box(objs[i], h_scale, w_scale);
    }
}
                                  ...
                                  int right)
{
    ...
    // Tinh toan song song su dung openmp
...
void print_faceobject(const FaceObject &obj, int i)
{
    fprintf(stderr, "[Face %d]: (prob: %.5f, x: %.2f, y: %.2f, width: %.2f, height: %.2f)\n", i, obj.prob, obj.rect.x, obj.rect.y, obj.rect.width, obj.rect.height);
}

static void draw_faceobjects(const cv::Mat &bgr,
                             const std::vector<FaceObject> &faceobjects)
{
    ...
    for (size_t i = 0; i < faceobjects.size(); i++)
    {
        ...
        print_faceobject(obj, i + 1);

        cv::rectangle(image, obj.rect, cv::Scalar(255, 0, 0));

        cv::circle(image, obj.landmark[0], 1, cv::Scalar(0, 0, 255), -1);
        cv::circle(image, obj.landmark[1], 1, cv::Scalar(0, 0, 255), -1);
        cv::circle(image, obj.landmark[2], 1, cv::Scalar(0, 0, 255), -1);
        cv::circle(image, obj.landmark[3], 1, cv::Scalar(0, 0, 255), -1);
        cv::circle(image, obj.landmark[4], 1, cv::Scalar(0, 0, 255), -1);
    }
